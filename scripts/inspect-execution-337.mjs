...
.headers off
.mode json
SELECT data FROM execution_data WHERE executionId = ${execId};
`;
  const localSqlFile = path.resolve(process.cwd(), 'scripts/temp_inspect_337.sql');
  
  try {
    fs.writeFileSync(localSqlFile, sqlQuery, 'utf8');
    execSync(`gcloud compute scp ${localSqlFile} n8n-server:/tmp/inspect_337.sql --zone=us-central1-a`);
    const output = execSync(`gcloud compute ssh n8n-server --zone=us-central1-a --command=\"sudo sqlite3 /var/lib/docker/volumes/n8n-docker_n8n_data/_data/database.sqlite < /tmp/inspect_337.sql\"`, { maxBuffer: 30 * 1024 * 1024 }).toString().trim();

    if (!output) {
      console.log(`No data found for execution ${execId}`);
      return;
    }
    ...
    console.log(`=== UNFLATTENED EXECUTION ${execId} ===`);
    if (fullExec.resultData.error) {
      console.log('ERROR:', JSON.stringify(fullExec.resultData.error, null, 2));
    }
