...
const nodes = [
  {
    ...
    "id": "b4e60a92-ef47-44c6-be86-d300eb0576a5",
    "name": "Webhook",
...
async function main() {
  console.log(`\n${colors.cyan}${colors.bold}🔧 CREATING AND INJECTING SEMI-AUTOMATED BOOKING WORKFLOW...${colors.reset}\n`);
  ...
  try {
    ...
    const sqlQuery = `
      ...
      -- 2. Insert workflow history (which now references the existing workflow_entity.id)
    ...
    const sqlFile = path.resolve(process.cwd(), 'scripts/create_semi_automated.sql');
    fs.writeFileSync(sqlFile, sqlQuery, 'utf8');

    console.log(`- Copying SQL file to Postgres container...`);
    execSync(`docker cp ${sqlFile} n8n-autoscaling-postgres-1:/tmp/create_semi_automated.sql`);

    console.log(`- Executing database insert...`);
    const output = execSync(`docker exec -i n8n-autoscaling-postgres-1 psql -U postgres -d n8n -f /tmp/create_semi_automated.sql`).toString();
    ...
    console.log(`\n${colors.green}${colors.bold}✔ SEMI-AUTOMATED BOOKING WORKFLOW CREATED SUCCESSFULLY!${colors.reset}\n`);
