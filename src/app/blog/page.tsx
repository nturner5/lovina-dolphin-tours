    ...
    <main className="bg-cloud-dancer min-h-screen px-6 pt-12 pb-24 lg:pt-16 lg:px-12">
      <div className="max-w-6xl mx-auto">
        <div className="mb-20 text-center">
          <h1 className="text-6xl lg:text-7xl font-serif text-deep-indigo mb-6">{t('blogTitle', locale)}</h1>
          <p className="text-lg text-deep-indigo/60 font-light max-w-xl mx-auto italic">{t('blogSubtitle', locale)}</p>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-16">
          {posts.map((post: any) => (
            <Link key={post._id} href={hrefFor(`/blog/${post.slug.current}`)} className="group">
          ...
          {posts.length === 0 && (
            <div className="col-span-full text-center py-32 border-2 border-dashed border-deep-indigo/10 rounded-[3rem]">
              <p className="font-serif italic text-2xl text-deep-indigo/20">{t('blogEmpty', locale)}</p>
            </div>
          )}
        </div>
      </div>
